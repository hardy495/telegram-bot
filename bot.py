        ...
        if state in ["asking_name", "waiting_balance"]:
            ...
            if amt == 0:
                await event.message.answer(
                    f"✅ Бронь найдена!\n\n"
                    f"🔑 *Заселение у нас дистанционное* — вы заселяетесь самостоятельно через минисейф.\n"
                    f"Все инструкции придут после подтверждения документов.\n\n"
                    f"Вы уже полностью оплатили бронирование! 🎉\n\n"
                    f"Для оформления нам потребуется:\n\n"
                    f"📄 Фото паспорта на чьё имя оформлена бронь (лицевая сторона)\n\n"
                    f"💰 Залог: {DEPOSIT} руб.\n"
        ...
        if state == "verified" and text:
            ...
            intent_resp = claude.messages.create(
                ...
                messages=[{"role":"user","content":
                    ...
                    f"ВЫЕЗД — только если гость сообщает что УЖЕ выехал или выезжает прямо сейчас.\n"
                    f"ПРОДЛЕНИЕ — если гость хочет выехать ПОЗЖЕ, продлить или сделать новую бронь.\n"
                    f"ДРУГОЕ — всё остальное (вопросы, жалобы, просьбы).\n\n"
