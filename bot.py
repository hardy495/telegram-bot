...
def start_max_bot():
    ...
    async def mm(event: MC):
        ...
        if state == "verified" and text:
            ...
            intent_resp = claude.messages.create(
                ...
                messages=[{"role":"user","content":
                    ...
                    f"ВЫЕЗД — только если гость сообщает что УЖЕ выехал или выезжает прямо сейчас (например: 'мы выехали', 'уже съехали', 'покидаем квартиру сейчас').\n"
                    f"ПРОДЛЕНИЕ — если гость хочет выехать ПОЗЖЕ обычного времени, продлить проживание, остаться ещё, сделать новую бронь (например: 'хочу выехать в 14:00', 'можно остаться до вечера', 'хотим продлить', 'хочу новую бронь').\n"
                    f"ДРУГОЕ — всё остальное.\n\n"
        ...
        if state == "waiting_requisites":
            # Гость прислал реквизиты после выезда
            apt_name = max_apt.get(uid, "неизвестный апартамент")
            await tg_admin(f"💳 Реквизиты для возврата залога (MAX)\n\nАпартамент: {apt_name}\nГость: {un}\n\nРеквизиты:\n{text}")
            await event.message.answer(
                "Благодарим вас за реквизиты! 🙏\n\n"
                "Залог вернём сегодня до 00:00. ✅\n\n"
